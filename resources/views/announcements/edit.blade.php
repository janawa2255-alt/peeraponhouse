...
<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-neutral-200">แก้ไขประกาศ</h1>
    ...
    <div class="bg-neutral-800 rounded-lg border border-neutral-700 p-6">
        <form action="{{ route('backend.announcements.update', $announcement->announcement_id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <!-- รูปภาพใหม่ -->
                <div>
                    <label class="block text-sm font-medium text-neutral-300 mb-2">เปลี่ยนรูปภาพ (ถ้าต้องการ)</label>
                    <input type="file" 
                           name="image" 
                           accept="image/*"
                ...
                <div>
                    ...
                    <select name="status" 
                            class="w-full px-4 py-2 bg-neutral-900 border border-neutral-600 rounded-lg text-neutral-200 focus:border-orange-500 focus:outline-none"
                            required>
                        <option value="1" {{ old('status', $announcement->status) == 1 ? 'selected' : '' }}>เปิดใช้งาน</option>
                        <option value="0" {{ old('status', $announcement->status) == 0 ? 'selected' : '' }}>ปิดใช้งาน</option>
                    </select>
                    @error('status')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
